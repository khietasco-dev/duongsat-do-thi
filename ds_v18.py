# -*- coding: utf-8 -*-
"""Ban 18 — BON TRANG TIENG TRUNG + CHAN TRANG (dot 1 cua bo tieng Trung).

/zh/quy-trinh/ · /zh/kiem-toan/ · /zh/tu-van/ · /zh/lien-he/  -> trang 46 -> 50.

⚠ RANG BUOC THUAT NGU (asco_thuat_ngu.json):
  竣工决算 = quyet toan DU AN, KHONG phai 汇算清缴 (quyet toan thue)
  亿 = 100 TRIEU. Mot ty dong = 十亿越南盾 — may tinh phi dung dung don vi nay.
  抵扣 (khau tru VAT) KHAC 扣除 (duoc tru thue TNDN)
  Hop dong kiem toan = 审计业务约定书, khong phai 审计合同
  Chung kien kiem ke = 监盘, khong phai 盘点
"""
...
from ds_zh_qt import QT, QT_GD, QT_NGAN, QT_HO, KT, KT_VS, KT_SS, KT_PH, KT_CD, KT_KL
from ds_zh_tv import TV, LH, CHAN_ZH
...
def trang_quy_trinh():
  ...
  <div class="ct">
    ...
    ng = ''.join('<tr><td><span class="the-loc %s" style="font-size:13px">%s 轨道</span></td>'
...
<div class="than"><div class="wrap">
  ...
  <div class="bang-boc"><table>
    <thead><tr><th style="width:104px">轨道</th><th style="width:26%%">项目类型</th>
...
""" % dict(h_gd=E(QT['h_gd']), gd=gd, h_ngan=E(QT['h_ngan']), ngan_lede=E(QT['ngan_lede']),
           ...
           ke=QT['ke'] % ('<a href="@/zh/kiem-toan/index.html">%s</a>' % E(QT['ke_lk'])))
    return than, [{"@context": "https://schema.org", "@type": "Article",
                   "headline": QT['h1'], "description": QT['mt'], "inLanguage": "zh"}]
...
""" % dict(h_sh=E(KT['h_sh']), sh=E(KT['sh']), h_vs=E(KT['h_vs']), vs=vs,
           ...
           h_bang=E(KT['h_bang']), bang=_bang_dm_zh(), bang_ghi=KT['bang_ghi'],
           h_nho=E(KT['h_nho']), nho=KT['nho'],
           h_sh2=E(KT['h_sh2']), sh2=KT['sh2'], sh3=E(KT['sh3']), h_bt=E(KT['h_bt']),
           bt=KT['bt'] % ('<a href="@/zh/tu-van/index.html">%s</a>' % E(KT['bt_lk'])))
    return than, [{"@context": "https://schema.org", "@type": "Article",
                   "headline": KT['h1'], "description": KT['mt'], "inLanguage": "zh"}]


# May tinh phi ban tieng Trung.
# ⚠ Don vi "ty dong" phai viet la 十亿越南盾 — 亿 chi la 100 trieu.
B.than_js['zh/kiem-toan'] = """
<script>
(function(){
 ...
 function tyLe(g,B){
  if(g<=MOC[0]) return {k:B[0], ct:'价值 ≤ 50 亿越南盾 — 费率 '+n(B[0])+'%'};
  if(g>=MOC[MOC.length-1]) return {k:B[B.length-1], ct:'价值 ≥ 10 万亿越南盾 — 费率 '+n(B[B.length-1])+'%'};
 ...
 function tien(x){ return Math.round(x).toLocaleString('zh-CN')+' 越南盾'; }
 ...
 function tinh(){
  ...
  if(!(so>0)){ kq.innerHTML='<p class="loi">请输入大于零的数值。</p>'; return; }
  var dong=so*hs, ty=dong/1e9, a=tyLe(ty,KT), b=tyLe(ty,TT), he=1, mo=[];
  if(tb.checked){ he*=0.7; mo.push('×70% — 设备费 ≥ 50%'); }
  if(bt.checked){ he*=0.5; mo.push('×50% — 补偿与安置费用'); }
  ...
  kq.innerHTML=
   '<div class="dong"><span>计费基数</span><span>'+tien(dong)+'</span></div>'+
   '<div class="dong"><span>适用费率</span><span>'+n(a.k)+'%</span></div>'+
   (mo.length?'<div class="dong"><span>调整系数</span><span>'+mo.join(' · ')+'</span></div>':'')+
   '<div class="dong"><span>税前审计费</span><span>'+tien(pkt)+(chan?' *':'')+'</span></div>'+
   '<div class="dong"><span>增值税 '+(tvat*100)+'%</span><span>'+tien(thue)+'</span></div>'+
   '<div class="to"><span class="nhan">审计费上限</span><div class="so">'+tien(tong)+'</div>'+
     '<div class="phu">已含增值税</div></div>'+
   '<div class="cthuc">'+a.ct+'</div>'+
   (chan?'<p class="small" style="margin-top:9px">* 已按第 20 条第 1 款 b 项适用一百万越南盾的下限。</p>':'')+
   '<div class="dong" style="margin-top:15px;padding-top:13px;border-top:1px solid var(--vien)">'+
     '<span>决算审查与批准费</span><span>'+tien(ptt)+(chan2?' *':'')+'</span></div>'+
   '<p class="small" style="margin-top:6px;margin-bottom:0">该项由审查机关收取，不是付给审计方的费用，'+
     '且不加计增值税。</p>';
 }
...
def trang_tu_van():
    ...
    the = ''.join(V17.V16.V15._the(d, i + 1, 'zh') for i, d in enumerate(V14.DICH_VU))
...
""" % dict(h_ba=E(TV['h_ba']), ba=ba, h_dv=E(TV['h_dv']), dv_lede=E(TV['dv_lede']),
           dv_ghi=TV['dv_ghi'] % ('<a href="@/zh/dich-vu/index.html">%s</a>' % E(TV['dv_lk'])),
    ...
    return than, [{"@context": "https://schema.org", "@type": "ContactPage",
                   "name": TV['h1'], "description": TV['mt'], "inLanguage": "zh"}]


B.than_js['zh/tu-van'] = B.than_js.get('tu-van', '')
...
def trang_lien_he():
    ...
    for i, (nhan, ten, mo, dm, lk) in enumerate(LH['ba']):
        if i == 0:
            them = ('<p style="margin-top:10px"><a href="@/zh/tu-van/index.html" '
            ...
            them = ('<p class="small" style="margin-top:10px">%s</p>'
                    % (LH['dl_ghi'] % ('<a href="@/zh/tu-van/index.html">%s</a>' % E(LH['dl_lk']))))
...
def khung(slug, tieude, mota, than, jsonld=None, tang='trong', lang='vi'):
    ...
    return V17._chan(h, 'zh', goc)


B.khung = khung

TRANG = list(V17.TRANG) + [
    ('zh/quy-trinh', 'zh', QT['td'], QT['mt'], trang_quy_trinh, 'trong'),
    ('zh/kiem-toan', 'zh', KT['td'], KT['mt'], trang_kiem_toan, 'trong'),
    ('zh/tu-van', 'zh', TV['td'], TV['mt'], trang_tu_van, 'trong'),
    ('zh/lien-he', 'zh', LH['td'], LH['mt'], trang_lien_he, 'trong'),
]
